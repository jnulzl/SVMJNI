...
{
#if _MSC_VER
    return _aligned_malloc(size, AI_MALLOC_ALIGN);
    ...
    return memalign(AI_MALLOC_ALIGN, size + AI_MALLOC_OVERREAD);
#else
    unsigned char* udata = (unsigned char*)malloc(size + sizeof(void*) + AI_MALLOC_ALIGN + AI_MALLOC_OVERREAD);
    if (!udata)
        return 0;
    unsigned char** adata = alignPtr((unsigned char**)udata + 1, AI_MALLOC_ALIGN);
    adata[-1] = udata;
    return adata;
#endif
}
