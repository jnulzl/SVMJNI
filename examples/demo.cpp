...
float testSVR1D(const FUNCPtr& func_ptr, int kernelType, int train_num, int dim = 1)
{
    ...
    /** Train the SVM */
    svm.svm_train(X.data(), Y.data(), train_num, svmConfig);

    /** Predict the SVM */
    svm.svm_predict(X.data(), train_num);

    /** Get SVM predict */
    size_t num = -1;
    const float* pred = svm.get_result(num);

    /** Print predict */
//    for(size_t idx = 0; idx < num; ++idx)
//    {
//        std::cout << X[idx] << " --> " << pred[idx] << "(" << func_ptr(X[idx]) << ")" << std::endl;
//    }
...
float testSVR2D(const FUNC2DPtr& func_ptr, int kernelType, int train_num, int dim = 2)
{
    ...
    /** Train the SVM */
    svm.svm_train(X.data(), Y.data(), Y.size(), svmConfig);

    /** Predict the SVM */
    svm.svm_predict(X.data(), Y.size());
...
//    for(size_t idx = 0; idx < num; ++idx)
//    {
//        std::cout << "(" << X[dim * idx] << ", " << X[dim * idx + 1] << ") --> " << pred[idx] << "(" << Y[idx] << ")" << std::endl;
//    }
    float mse_val = mse(Y.data(), pred, num);
    svm.release();
    return mse_val;
